<template>
    <div>
        <el-dialog :title="goosInfo.isAdd ? '商品添加' : '商品编辑'" :visible.sync="goosInfo.isShow" center :before-close="cancel"
            @opened="createEdit">
            <el-form :model="form" label-width="80px" :rules="rules" ref="goodsInfo">
                ...
                <el-form-item label="图片" prop="img" limit="1" :auto-upload="false">
                ...
                <el-form-item label="商品规格" prop="specsid">
                    <el-select @change="changeSpecs(false)" v-model="form.specsid" placeholder="请选择">
                        ...
                        <el-option v-for="item in specslist" :label="item.specsname" :value="item.id">
                        </el-option>
...
export default {
    ...
    data() {
        return {
            ...
            //表单验证
            rules: {
                first_cateid: [{ required: true, message: "请选择一级分类名称", trigger: "change" }],
                second_cateid: [{ required: true, message: "请选择二级分类名称", trigger: "blur" }],
                goodsname: [
                ...
                specsattr: [{ required: true, message: "请选择商品属性 ", trigger: "blur" }],
